...
{   
   ...
   int L; /*dimensione della stringa ritornata dal figlio*/
	if (argc != 2)
	...
	{   
		...
		while (read(fd, &(mess[L]), 1) != 0) /* il contenuto del file e' tale che in mess ci saranno 4 caratteri e il terminatore di linea */
		{   
			L++;
			if(mess[L-1] == '\n'){
    			...
    			mess[L-1]='\0'; 
				write(piped[1], &L, sizeof(L)); /*figlio comunica lunghezza della stringa al padre*/
	...
	while (read(piped[0], &L, sizeof(L)) != 0)  /* questo ciclo avra' termine appena il figlio terminera' dato che la read senza piu' scrittore tornera' 0! */
	{ 	
			if(read(piped[0], mess, L) != L){
